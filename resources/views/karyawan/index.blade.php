...
@section('content')
        ...
        <form action="{{ route('karyawan.exportQrCode') }}" method="POST" target="_blank" id="exportQrCodeForm">
            ...
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-outline-primary" id="select-all-visible">Pilih Semua yang
                        Tampil</button>
                    <button type="button" class="btn btn-outline-secondary" id="clear-selected">Hapus Pilihan</button>
                </div>
            </div>
        </form>
    @endif
    ...
    <div id="karyawan-container">
        <div class="row row-cards">
                                ...
                                <div class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                    @haspermission('karyawan.detail')
                                        <button type="button" class="dropdown-item detail-button"
                                            data-nip="{{ $karyawan->nip }}"><i
                                                class="ti ti-info-circle me-2"></i>Detail</button>
                                    @endhaspermission
                                    @haspermission('karyawan.edit')
                                        <button type="button" class="dropdown-item edit-button"
                                            data-uuid="{{ $karyawan->uuid }}"><i class="ti ti-pencil me-2"></i>Edit</button>
                                    @endhaspermission
                                    @haspermission('karyawan.delete')
                                        <button type="button" class="dropdown-item delete-button"
                                            data-uuid="{{ $karyawan->uuid }}"><i class="ti ti-trash me-2"></i>Delete</button>
                                    @endhaspermission
                                </div>
                            </div>
                        </div>
            ...
            @endforelse
            <div class="d-flex justify-content-end mt-4">{{ $karyawans->links() }}</div>
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.karyawan-checkbox', function() {
                ...
                } else selectedNips = selectedNips.filter(item => item !== nip);
                syncSelectedInput();
            });
            ...
            $(document).on('submit', '#exportQrCodeForm', function(e) {
                ...
                if (selectedNips.length === 0) {
                    ...
                    else alert('Pilih minimal satu karyawan untuk export');
                }
            });
            ...
            $('#editForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    error: function(err) {
                        ...
                        } else showErrorToast(err.responseJSON?.message ??
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const formImport = document.getElementById('form-import-karyawan'),
                ...
                btnTest = document.getElementById('btn-test-import'),
                btnImport = document.getElementById('btn-import-karyawan'),
                validationResult = document.getElementById('import-validation-result'),
                previewWrapper = document.getElementById('import-preview-wrapper'),
                previewBody = document.getElementById('import-preview-body'),
                testImportUrl = @json(route('karyawan.import.test')),
            ...
            function renderPreview(data) {
                ...
                data.forEach(function(item) {
                    ...
                    const actionBadge = item.action === 'INSERT' ?
                        ...
                        statusBadge = item.valid ? '<span class="badge bg-success">Valid</span>' :
...
@push('modal')
            ...
            <form id="form-import-karyawan" action="{{ route('karyawan.import') }}" method="POST"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="mb-3"><label class="form-label">Upload File Excel</label><input type="file"
                                name="file" id="import-file" class="form-control" accept=".xlsx,.xls,.csv"
                                required><small class="text-muted">Format file: .xlsx, .xls, .csv</small></div>
                        <div class="alert alert-info"><strong>Catatan:</strong>Pastikan format header file sesuai</div>
                        <p class="mb-3">Belum mempunyai template import? <a
                                href="{{ asset('assets/static/template/template-import-karyawan.xlsx') }}"
                                download>Download template</a></p>
                    ...
                    <div class="modal-footer"><button type="button" class="btn btn-outline-danger"
                            data-bs-dismiss="modal">Batal</button><button type="button" id="btn-test-import"
                            class="btn btn-outline-primary"><i class="ti ti-check me-1"></i>Tes / Validasi</button><button
                            type="submit" id="btn-import-karyawan" class="btn btn-success" disabled><i
                                class="ti ti-upload me-1"></i>Import Sekarang</button></div>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <form action="{{ route('karyawan.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title">Karyawan Baru</h5><button type="button" class="btn-close"
                            data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                                ...
                                <select class="form-select" name="uuid_posisi">
                                    ...
                                    @foreach ($posisi as $pos)
                                        <option value="{{ $pos->uuid }}"
                                            {{ old('uuid_posisi') == $pos->uuid ? 'selected' : '' }}>{{ $pos->nama_posisi }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('uuid_posisi')
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">NIP</label><input type="text"
                                    class="form-control" name="nip" value="{{ old('nip') }}"
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">Nama Depan</label><input type="text"
                                    class="form-control" name="nama_depan" value="{{ old('nama_depan') }}"
                                    placeholder="Masukkan nama depan" required>
                                @error('nama_depan')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="col-md-6 mb-3"><label class="form-label">Nama Belakang</label><input
                                    type="text" class="form-control" name="nama_belakang"
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">No HP</label><input type="text"
                                    class="form-control" name="no_hp" value="{{ old('no_hp') }}"
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">Tempat Lahir</label><input
                                    type="text" class="form-control" name="tempat_lahir"
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">Tanggal Lahir</label><input
                                    type="date" class="form-control" name="tgl_lahir"
                                    value="{{ old('tgl_lahir') }}">
                                @error('tgl_lahir')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="col-md-12 mb-3"><label class="form-label">Alamat</label>
                    ...
                    <div class="modal-footer"><button type="button" class="btn btn-outline-danger"
                            ...
                            class="btn btn-primary ms-auto">Simpan</button></div>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="modal modal-blur fade" id="modal-edit" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <form id="editForm" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit Karyawan</h5><button type="button" class="btn-close"
                            data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3"><label class="form-label">Foto / Avatar</label><input
                                    type="file" id="edit_avatar" class="form-control" name="avatar"
                                    accept="image/*"><small class="text-danger" id="edit_avatar_error"></small></div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Divisi</label>
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">NIP</label><input type="text"
                                    id="edit_nip" name="nip" class="form-control"><small class="text-danger"
                                    id="edit_nip_error"></small></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Email</label><input type="email"
                                    id="edit_email" name="email" class="form-control" required><small
                                    class="text-danger" id="edit_email_error"></small></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Nama Depan</label><input type="text"
                                    id="edit_nama_depan" name="nama_depan" class="form-control" required><small
                                    class="text-danger" id="edit_nama_depan_error"></small></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Nama Belakang</label><input
                                    type="text" id="edit_nama_belakang" name="nama_belakang"
                                    class="form-control"><small class="text-danger"
                                    id="edit_nama_belakang_error"></small></div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Jenis Kelamin</label>
                            ...
                            <div class="col-md-6 mb-3"><label class="form-label">No HP</label><input type="text"
                                    id="edit_no_hp" name="no_hp" class="form-control"><small class="text-danger"
                                    id="edit_no_hp_error"></small></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Tempat Lahir</label><input
                                    type="text" id="edit_tempat_lahir" name="tempat_lahir"
                                    class="form-control"><small class="text-danger" id="edit_tempat_lahir_error"></small>
                            </div>
                            <div class="col-md-6 mb-3"><label class="form-label">Tanggal Lahir</label><input
                                    type="date" id="edit_tgl_lahir" name="tgl_lahir" class="form-control"><small
                                    class="text-danger" id="edit_tgl_lahir_error"></small></div>
                            <div class="col-md-6 mb-3"><label class="form-label">Status</label><select id="edit_status"
                                    name="status" class="form-select">
                                    <option value="1">Aktif</option>
                                    <option value="0">Nonaktif</option>
                                </select><small class="text-danger" id="edit_status_error"></small></div>
                            <div class="col-md-12 mb-3"><label class="form-label">Alamat</label>
                                <textarea id="edit_alamat" name="alamat" class="form-control" rows="3"></textarea><small class="text-danger" id="edit_alamat_error"></small>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer"><button type="button" class="btn btn-outline-danger"
    ...
    <div class="modal modal-blur fade" id="modal-detail" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Detail Karyawan</h5><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row align-items-center mb-4">
                        <div class="col-md-3 text-center"><span id="detail_avatar"
                        ...
                        <div class="col-md-3 text-center"><img id="detail_qr_code"
                                src="{{ asset('assets/static/avatars/default.jpg') }}" alt="QR Code"
